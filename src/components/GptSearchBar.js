...
searchTextValue === ''  ? seterrormessage("please enter your movie category like funny english movies,action movies...."): seterrormessage(null); 
 ...
 setsearchloading(false);
  ...
  return (
    ...
    <div className="flex justify-center ">
        ...
        <form onSubmit={(e)=>{e.preventDefault()}} className="mt-[15%] bg-black w-1/2  rounded-lg  pr-[20px] " >
        <h1 className="text-white text-center p-4 pt-8 text-3xl">GPT Search</h1>
        <div className="flex  items-center">
            <input  ref={searchText} className="px-4 py-4 m-6 col-span-8 rounded-lg mr-12 ml-10 w-9/12" type="text" alt="Gptsearchbar" placeholder={lang[langKey].gptPlaceholder} onKeyDown={handleEntergpt}/>
            <button className="px-12 py-4   col-span-4  bg-red-600 text-white rounded-lg w-3/12  text-center" onClick={()=>handleGptSearch()}> {searchloading ? <svg aria-hidden="true" class="w-6 h-6 mx-4 text-gray-200 animate-spin dark:text-gray-600 fill-red-400" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    </svg>: lang[langKey].search}</button>
    </div>
